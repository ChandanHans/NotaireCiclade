...
def create_tag_and_release(headers, repo, tag, commit_sha, release_title, file_path):
    ...
    response = requests.post(f"https://api.github.com/repos/{repo}/releases", headers=headers, json=release_data)
    if response.status_code == 201:
...
def main():
    ...
    commit_sha = os.getenv("COMMIT_SHA")  # This needs to be dynamically determined or passed in
    repo = os.getenv("REPO")
